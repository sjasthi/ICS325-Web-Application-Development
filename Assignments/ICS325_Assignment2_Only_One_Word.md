...
The filler characters must preserve the **relative frequency of letters in the supplied word**.

For example, if the word is:

`HELLO`

the available letter pattern is based on:
...
Therefore, `L` should occur approximately twice as often as each of the other letters when filler characters are generated.

For a word such as:

`BALLOON`

the filler generation must similarly reflect the repeated occurrences of `L` and `O`.

One acceptable approach is to randomly select filler letters from the original word string. Because repeated letters appear multiple times in the source word, their frequency will naturally be preserved.
